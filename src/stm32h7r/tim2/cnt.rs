...
/**Value depends on IUFREMAP in TIMx_CR1. If UIFREMAP = 0 CNT\[31\]: Most significant bit of counter value If UIFREMAP = 1 UIFCPY: UIF Copy This bit is a read-only copy of the UIF bit of the TIMx_ISR register
...
///Field `UIFCPY` reader - Value depends on IUFREMAP in TIMx_CR1. If UIFREMAP = 0 CNT\[31\]: Most significant bit of counter value If UIFREMAP = 1 UIFCPY: UIF Copy This bit is a read-only copy of the UIF bit of the TIMx_ISR register
...
///Field `UIFCPY` writer - Value depends on IUFREMAP in TIMx_CR1. If UIFREMAP = 0 CNT\[31\]: Most significant bit of counter value If UIFREMAP = 1 UIFCPY: UIF Copy This bit is a read-only copy of the UIF bit of the TIMx_ISR register
...
impl R {
    ///Bits 0:31 - Least significant part of counter value Non-dithering mode (DITHEN = 0) The register holds the counter value. Dithering mode (DITHEN = 1) The register holds the non-dithered part in CNT\[30:0\]. The fractional part is not available.
    #[inline(always)]
