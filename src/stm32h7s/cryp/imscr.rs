...
///Field `INIM` reader - Input FIFO service interrupt mask This bit enables or disables (masks) the CRYP input FIFO service interrupt generation when INRIS is set.
pub type INIM_R = crate::BitReader;
///Field `INIM` writer - Input FIFO service interrupt mask This bit enables or disables (masks) the CRYP input FIFO service interrupt generation when INRIS is set.
pub type INIM_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `OUTIM` reader - Output FIFO service interrupt mask This bit enables or disables (masks) the CRYP output FIFO service interrupt generation when OUTRIS is set.
pub type OUTIM_R = crate::BitReader;
///Field `OUTIM` writer - Output FIFO service interrupt mask This bit enables or disables (masks) the CRYP output FIFO service interrupt generation when OUTRIS is set.
