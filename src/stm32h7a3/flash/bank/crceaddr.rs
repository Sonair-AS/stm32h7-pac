...
///Field `CRC_END_ADDR` reader - CRC end address on bank 1 CRC_END_ADDR is used when CRC_BY_SECT is cleared to 0. It must be programmed to the end address of the bank 1 memory area on which the CRC calculation is performed
pub type CRC_END_ADDR_R = crate::FieldReader<u32>;
///Field `CRC_END_ADDR` writer - CRC end address on bank 1 CRC_END_ADDR is used when CRC_BY_SECT is cleared to 0. It must be programmed to the end address of the bank 1 memory area on which the CRC calculation is performed
