...
///Field `PMTIS` reader - PMT Interrupt Status This bit is set when a Magic packet or Wake-on-LAN packet is received in the power-down mode (RWKPRCVD and MGKPRCVD bits in ETH_MACPCSR register). This bit is cleared when Bits\[6:5\] are cleared because of a Read operation to the (ETH_MACPCSR).
pub type PMTIS_R = crate::BitReader;
///Field `LPIIS` reader - LPI Interrupt Status This bit is set for any LPI state entry or exit in the MAC Transmitter or Receiver. This bit is cleared when the TLPIEN bit of is read.
...
///Field `MMCRXIS` reader - MMC Receive Interrupt Status This bit is set high when an interrupt is generated in the (ETH_MMC_RX_INTERRUPT). This bit is cleared when all bits in this interrupt register are cleared.
pub type MMCRXIS_R = crate::BitReader;
///Field `MMCTXIS` reader - MMC Transmit Interrupt Status This bit is set high when an interrupt is generated in the (ETH_MMC_TX_INTERRUPT). This bit is cleared when all bits in this interrupt register are cleared.
...
/**Field `RXSTSIS` reader - Receive Status Interrupt This bit indicates the status of received packets. This bit is set when the RWT bit is set in the . This bit is cleared when the corresponding interrupt source bit is read (or corresponding interrupt source bit is written to 1 when RCWE bit of is set) in the ETH_MACISR register.

<div class="warning">The field is <b>cleared</b> (set to zero) following a read operation.</div>*/
pub type RXSTSIS_R = crate::BitReader;
///Field `RXSTSIS` writer - Receive Status Interrupt This bit indicates the status of received packets. This bit is set when the RWT bit is set in the . This bit is cleared when the corresponding interrupt source bit is read (or corresponding interrupt source bit is written to 1 when RCWE bit of is set) in the ETH_MACISR register.
...
impl R {
    ...
    ///Bit 4 - PMT Interrupt Status This bit is set when a Magic packet or Wake-on-LAN packet is received in the power-down mode (RWKPRCVD and MGKPRCVD bits in ETH_MACPCSR register). This bit is cleared when Bits\[6:5\] are cleared because of a Read operation to the (ETH_MACPCSR).
    #[inline(always)]
    pub fn pmtis(&self) -> PMTIS_R {
        PMTIS_R::new(((self.bits >> 4) & 1) != 0)
    }
    ///Bit 5 - LPI Interrupt Status This bit is set for any LPI state entry or exit in the MAC Transmitter or Receiver. This bit is cleared when the TLPIEN bit of is read.
    #[inline(always)]
    ...
    ///Bit 9 - MMC Receive Interrupt Status This bit is set high when an interrupt is generated in the (ETH_MMC_RX_INTERRUPT). This bit is cleared when all bits in this interrupt register are cleared.
    #[inline(always)]
    pub fn mmcrxis(&self) -> MMCRXIS_R {
        MMCRXIS_R::new(((self.bits >> 9) & 1) != 0)
    }
    ///Bit 10 - MMC Transmit Interrupt Status This bit is set high when an interrupt is generated in the (ETH_MMC_TX_INTERRUPT). This bit is cleared when all bits in this interrupt register are cleared.
    #[inline(always)]
    ...
    ///Bit 14 - Receive Status Interrupt This bit indicates the status of received packets. This bit is set when the RWT bit is set in the . This bit is cleared when the corresponding interrupt source bit is read (or corresponding interrupt source bit is written to 1 when RCWE bit of is set) in the ETH_MACISR register.
    #[inline(always)]
